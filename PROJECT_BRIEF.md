...
4. Who owns the endpoint numbers, and what will compute cost?
   - **Recommended default:** The experiment plan states the endpoint, estimator, seed policy, thresholds, and multiplicity correction. Review them there, once. Compute costs about 55 runs at gate time: seven end-to-end arms plus four meaningful frozen-head variants (deterministic, Gaussian, MDN, auxiliary task), five seeds each after the three-seed bring-up. End-to-end arms dominate the bill.
5. When do planning results enter the evaluation?
   - **Recommended default:** The endpoint has three parts: probe accuracy, known-forward cycle validity, and latent-planning success. Latent planning encodes the observation, rolls the learned forward predictor forward, and scores candidate actions against an encoded goal, so the representation sits inside the planning loop; a planner that bypassed it with true dynamics would measure nothing about the encoder. Collect all three in the same pass. A state-space planner with true dynamics is reported as a reference ceiling only. Learned-model control stays outside the endpoint until simpler evaluations show a benefit.
